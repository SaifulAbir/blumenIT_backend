...
from .models import *
from product.models import Product, Inventory, Specification, ShippingClass, ShippingCountry
...
class ProductSerializer(serializers.ModelSerializer):
    ...
    seller_user_id = serializers.CharField(
        source='seller.seller_user.id', read_only=True)

    class Meta:
        model = Product
        fields = ['id', 'thumbnail', 'title', 'price',
                  'total_quantity', 'seller', 'seller_user_id']
...
class CheckoutDetailsOrderItemSerializer(serializers.ModelSerializer):
    ...
    offer_discount_price = serializers.CharField(
        source='offer.discount_price', read_only=True)
    offer_discount_price_type = serializers.CharField(
        source='offer.discount_price_type', read_only=True)

    class Meta:
        model = OrderItem
        fields = ['id', 'product_title', 'product_thumb', 'product_price', 'product_sku', 'product_specification', 'quantity', 'unit_price', 'unit_price_after_add_warranty',
                  'total_price', 'product_warranty', 'product_warranty_title', 'product_vat', 'offer', 'offer_discount_price', 'offer_discount_price_type']
    ...
    def get_product_specification(self, obj):
        queryset = Specification.objects.filter(
            product=obj.product.id, is_active=True)
...
class CheckoutDetailsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'user', 'user_email', 'user_phone', 'order_id', 'order_date', 'delivery_date', 'order_status', 'order_items', 'delivery_address', 'payment_type',
                  'payment_title', 'product_price', 'coupon_discount_amount', 'sub_total', 'shipping_class', 'shipping_cost', 'total_price', 'vat_amount', 'warranty_price', 'discount_amount']

    def get_order_items(self, obj):
        queryset = OrderItem.objects.filter(order=obj)
        serializer = CheckoutDetailsOrderItemSerializer(
    ...
    def get_delivery_address(self, obj):
        try:
            if obj.delivery_address:
                ...
                serializer = CheckoutDetailsDeliveryAddressSerializer(
...
class ApplyCouponSerializer(serializers.ModelSerializer):
    class Meta:
        model = Coupon
        fields = ['id', 'code', 'min_shopping_amount', 'amount',
...
class OrderSerializer(serializers.ModelSerializer):
    class Meta:
        model = Order
        fields = ['id', 'user', 'order_id', 'product_count',
                  'order_date', 'order_status', 'total_price', 'discount_amount']
...
class OrderItemSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'product', 'quantity', 'unit_price',
                  'unit_price_after_add_warranty', 'subtotal']
...
class CheckoutSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = ['id', 'order_id', 'product_count', 'coupon', 'coupon_status', 'coupon_discount_amount', 'payment_type',
                  'shipping_class', 'shipping_cost', 'order_items', 'delivery_address', 'comment', 'discount_amount']
    ...
    def create(self, validated_data):
            ...
            for order_item in order_items:
                ...
                if offer:
                    ...
                    if discount_price_type == '%':
                        discount_amount_value = (
                            float(discount_price) * float(base_price)) / 100
                    elif discount_price_type == 'flat':
                        discount_amount_value = float(
                            discount_price) * float(quantity)
                ...
                if product_warranty:
                    ...
                    if warranty_value_type == 'PERCENTAGE':
                        warranty_value_data = (
                            float(base_price) * float(warranty_value)) / 100
                        unit_price_after_add_warranty = (
                            float(base_price) / float(quantity)) + warranty_value_data
                        ...
                        unit_price_after_add_warranty = (
                            float(base_price) / float(quantity)) + warranty_value
                    ...
                    if offer:
                        OrderItem.objects.create(order=order_instance, product=product, quantity=int(quantity), unit_price=unit_price, total_price=base_price,
                                                 product_warranty=product_warranty, unit_price_after_add_warranty=unit_price_after_add_warranty, offer=offer)
                    else:
                        OrderItem.objects.create(order=order_instance, product=product, quantity=int(
                            quantity), unit_price=unit_price, total_price=base_price, product_warranty=product_warranty, unit_price_after_add_warranty=unit_price_after_add_warranty)
                else:
                    if offer:
                        OrderItem.objects.create(order=order_instance, product=product, quantity=int(
                            quantity), unit_price=unit_price, total_price=base_price, offer=offer)
                    else:
                        OrderItem.objects.create(order=order_instance, product=product, quantity=int(
                            quantity), unit_price=unit_price, total_price=base_price)
                ...
                # vat calculation
                product_vat_value = Product.objects.filter(slug=product.slug)[
                    0].vat
                if product_vat_value:
                    vat_amount = (float(product_vat_value)
                                  * float(base_price)) / 100
                    vat_amount_list.append(vat_amount)

            Order.objects.filter(id=order_instance.id).update(vat_amount=sum(
                vat_amount_list), discount_amount=total_product_discount_amount, sub_total=sub_total)
        ...
        if coupon_status == True:
            ...
            if check_in_use_coupon_record:
                ...
                coupon_obj.update(
                    number_of_uses=number_of_uses + 1, is_active=True)
        ...
        try:
            total_product_discount_amount_data = float(
                total_product_discount_amount)
        except:
            total_product_discount_amount_data = 0.0

        grand_total_price = (sub_total_amount + vat_amount_data + shipping_cost_amount) - (
            coupon_discount_amount_data + total_product_discount_amount_data)
        ...
        try:
            delivery_days = ShippingClass.objects.get(
                id=order_instance.shipping_class.id).delivery_days
...
class ShippingClassDataSerializer(serializers.ModelSerializer):
    shipping_country_name = serializers.CharField(
        source='shipping_country.title', read_only=True)
    shipping_state_name = serializers.CharField(
        source='shipping_state.title', read_only=True)
    shipping_city_name = serializers.CharField(
        source='shipping_city.title', read_only=True)
    state_city_concate = serializers.SerializerMethodField(
        'get_state_city_concate')

    class Meta:
        model = ShippingClass
        fields = ['id', 'description', 'shipping_country', 'shipping_country_name', 'shipping_state', 'shipping_state_name',
                  'shipping_city', 'shipping_city_name', 'delivery_days', 'delivery_charge', 'state_city_concate']
